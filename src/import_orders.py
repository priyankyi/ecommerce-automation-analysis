...
ORDER_INPUT_DIR = PROJECT_ROOT / "data" / "input" / "orders"
MAPPING_PATH = PROJECT_ROOT / "config" / "order_column_mapping.json"
LOG_PATH = PROJECT_ROOT / "data" / "logs" / "order_import_log.csv"

RAW_TAB = "ORDER_RAW"
MASTER_TAB = "ORDER_MASTER"
...
def normalize_row(
    ...
    selling_price = get_value("selling_price")
    order_status = get_value("order_status") or "Unknown"
    order_date = get_value("order_date")
    dispatch_date = get_value("dispatch_date")
    delivered_date = get_value("delivered_date")
    return_status = get_value("return_status")
    cancellation_reason = get_value("cancellation_reason")
...
def get_existing_master_keys(sheets_service, spreadsheet_id: str) -> Dict[Tuple[str, str, str], int]:
    rows = get_sheet_values(sheets_service, spreadsheet_id, f"{MASTER_TAB}!A1:N")
    if len(rows) < 2:
        return {}
    ...
    if order_index is None or marketplace_index is None or sku_index is None:
        return {}
    ...
    for row_number, row in enumerate(rows[1:], start=2):
        ...
        if order_id and marketplace and sku_id and (order_id, marketplace, sku_id) not in existing:
            existing[(order_id, marketplace, sku_id)] = row_number
    return existing


def get_existing_raw_keys(sheets_service, spreadsheet_id: str) -> Dict[Tuple[str, str, str], int]:
    rows = get_sheet_values(sheets_service, spreadsheet_id, f"{RAW_TAB}!A1:O")
    if len(rows) < 2:
        return {}

    headers = rows[0]
    order_index = find_header_index(headers, "Raw_Order_ID")
    marketplace_index = find_header_index(headers, "Marketplace")
    sku_index = find_header_index(headers, "Raw_SKU")
    if order_index is None or marketplace_index is None or sku_index is None:
        return {}
        ...
        },
        {
            "setDataValidation": {
                ...
                "rule": {
                    "condition": {
                        "type": "ONE_OF_LIST",
                        "values": [{"userEnteredValue": value} for value in ORDER_STATUS_OPTIONS],
                    },
                    "showCustomUi": True,
                    "strict": True,
                },
...
def ensure_order_import_system() -> Dict[str, object]:
    ...
    existing_raw_keys = get_existing_raw_keys(sheets_service, spreadsheet_id)
    existing_master_keys = get_existing_master_keys(sheets_service, spreadsheet_id)
    ...
    for record in records:
        ...
        key = (master_values[0], master_values[1], master_values[2])
        ...
        log_rows.append(
            {
                ...
                "sku_id": key[2],
                "status": "created",
                "message": "Imported order row",
            }
        )

    append_rows(sheets_service, spreadsheet_id, RAW_TAB, filtered_raw_rows, "O")
    append_rows(sheets_service, spreadsheet_id, MASTER_TAB, filtered_master_rows, "N")
    ...
    print(f"  Rows written to ORDER_RAW: {len(filtered_raw_rows)}")
    print(f"  Rows written to ORDER_MASTER: {len(filtered_master_rows)}")
    ...
    return {
        ...
        "import_directory": str(ORDER_INPUT_DIR),
...
def main() -> None:
    result = ensure_order_import_system()
