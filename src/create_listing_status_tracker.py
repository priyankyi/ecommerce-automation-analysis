...
LOG_PATH = PROJECT_ROOT / "data" / "logs" / "listing_status_setup_log.csv"

MASTER_SKU_TAB = "MASTER_SKU"
TAB_NAME = "LISTING_STATUS"
...
def get_product_content_rows(sheets_service, spreadsheet_id: str) -> Dict[str, Dict[str, str]]:
    rows = get_sheet_values(sheets_service, spreadsheet_id, "PRODUCT_CONTENT!A1:ZZ")
    if len(rows) < 2:
        return {}
    ...
    for row in rows[1:]:
        sku_id = row[sku_index].strip() if sku_index < len(row) else ""
        if not sku_id or sku_id in lookup:
            continue
        lookup[sku_id] = {
...
def get_existing_listing_keys(sheets_service, spreadsheet_id: str) -> Dict[Tuple[str, str], int]:
    rows = get_sheet_values(sheets_service, spreadsheet_id, f"{TAB_NAME}!A1:L")
    if len(rows) < 2:
        return {}
...
def map_row(sku_id: str, marketplace: str, content_lookup: Dict[str, Dict[str, str]]) -> List[str]:
    content = content_lookup.get(sku_id, {})
    row = [""] * len(HEADERS)
    row[0] = sku_id
    row[1] = marketplace
    row[2] = ""
    row[3] = "Not Started"
    row[8] = "Medium"
    row[10] = content.get("Last_Updated", "")
    row[11] = ""
    return row
...
def append_missing_rows(
    ...
    content_lookup: Dict[str, Dict[str, str]],
    existing_listing_keys: Dict[Tuple[str, str], int],
    ...
    for sku_id in master_skus:
        for marketplace in MARKETPLACES:
            ...
            if key in existing_listing_keys:
                log_rows.append(
                    {
                        "timestamp": timestamp,
                        "sku_id": sku_id,
                        "marketplace": marketplace,
                        "row_number": existing_listing_keys[key],
                        "status": "skipped",
                        "message": "Listing row already exists",
                    }
                )
                continue

            rows_to_append.append(map_row(sku_id, marketplace, content_lookup))
            log_rows.append(
                {
                    "timestamp": timestamp,
                    "sku_id": sku_id,
                    ...
                    "row_number": "",
                    "status": "created",
                    "message": "Seeded listing tracker row",
                }
            )
        ...
        },
        {
            "setDataValidation": {
                ...
                "rule": {
                    "condition": {
                        "type": "ONE_OF_LIST",
                        "values": [{"userEnteredValue": value} for value in LISTING_STATUS_OPTIONS],
                    },
                    "showCustomUi": True,
                    "strict": True,
                },
...
def ensure_listing_status_tracker() -> Dict[str, object]:
    ...
    existing_headers = get_sheet_values(sheets_service, spreadsheet_id, f"{TAB_NAME}!A1:L1")
    ...
    content_lookup = get_product_content_rows(sheets_service, spreadsheet_id)
    existing_listing_keys = get_existing_listing_keys(sheets_service, spreadsheet_id)
    ...
    return {
        ...
        "expected_rows": len(master_skus) * len(MARKETPLACES),
        "log_path": str(LOG_PATH),
    }


def main() -> None:
    result = ensure_listing_status_tracker()
